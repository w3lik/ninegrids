local process = Process("slice_diff1_area2_P2")
process:onStart(function(this)
    this:next("interrupt")
    local gd = Game():GD()
    async.call(gd.me:owner(), function()
        UI_NinegridsInfo:info("info", 5, "盗贼忽然撤退了~")
    end)
    ...
    Game():npc("VillagerKid2-1", TPL_UNIT.NPC_Token, 1712, 4465, 180, function(evtUnit)
        evtUnit:name("乱逛的小屁孩")
        ...
        evtUnit:balloon({
            z = 200,
            interval = 0.01,
            message = { { tips = { "盗贼把去往古遗迹的路毁了", "遗迹里面有很多古怪的石头" } } }
        })
    end)
    Game():npc("VillagerWoman-B", TPL_UNIT.NPC_Token, 54, 4580, 90, function(evtUnit)
        evtUnit:name("左顾右盼的女村民")
            ...
            message = {
                {
                    tips = {
                        "！！！你怎么发现我的！",
                        Game():balloonKeyboardTips("揭穿他！")
                    },
                    call = function()
                        Game():digClear()
                        ProcessCurrent:next("slice_diff1_area2_P3")
                    end
                },
            }
        })
